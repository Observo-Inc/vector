...
#[cfg(any(feature = "sources-utils-net-udp", feature = "sources-utils-net-tcp"))]
impl SocketListenAddr {
    const fn as_resource(self, protocol: super::config::Protocol) -> super::config::Resource {
        match self {
            Self::SocketAddr(addr) => match protocol {
                super::config::Protocol::Tcp => super::config::Resource::tcp(addr),
                super::config::Protocol::Udp => super::config::Resource::udp(addr),
            },
            Self::SystemdFd(fd_offset) => super::config::Resource::SystemFdOffset(fd_offset),
        }
    }

    /// Gets this listen address as a `Resource`, specifically for TCP.
    #[cfg(feature = "sources-utils-net-tcp")]
    pub const fn as_tcp_resource(self) -> super::config::Resource {
        self.as_resource(super::config::Protocol::Tcp)
    }

    /// Gets this listen address as a `Resource`, specifically for UDP.
    #[cfg(feature = "sources-utils-net-udp")]
    pub const fn as_udp_resource(self) -> super::config::Resource {
        self.as_resource(super::config::Protocol::Udp)
    }
}
