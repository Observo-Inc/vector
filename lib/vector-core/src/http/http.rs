...
use std::{
    fmt, net::SocketAddr, task::{Context, Poll}, time::Duration,
};
...
use crate::{
    tls::{tls_connector_builder, MaybeTlsSettings, TlsError}, Result as Res,
};

use metrics::{counter, histogram};
use vector_common::internal_event::InternalEvent;
use crate::http::internal_events as http_client;
    ...
    B::Error: Into<crate::Error>,
{
    pub fn new(
        ...
        app_info: &vector_common::AppInfo,
    ) -> Result<HttpClient<B>, HttpError> {
        HttpClient::new_with_custom_client(tls_settings, proxy_config, &mut Client::builder(), app_info)
    }
    ...
    pub fn new_with_custom_client(
        ...
        let user_agent = HeaderValue::from_str(&format!("{}/{}", app_info.name, app_info.version))
...
/// Configuration of stateless authentication strategies for HTTP requests.
///
...
pub type Auth = StatelessAuth;

pub trait MaybeAuth: Sized {
    fn choose_one(&self, other: &Self) -> Res<Self>;
}
...
pub mod tests {
    ...
    use vector_common::AppInfo;

    use crate::inet_test_util::next_addr;
    ...
    async fn test_max_connection_age_service_with_hyper_server() {
        ...
        let app_info: AppInfo = vector_common::AppInfo{
            name: "test",
