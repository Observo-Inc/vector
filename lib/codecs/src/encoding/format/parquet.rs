...
#[derive(Debug)]
pub struct ParquetDropNotice {
    foo: &'static str
}

impl InternalEvent for ParquetDropNotice {
    fn emit(self) {
        metrics::counter!("parquet_schema_mismatch_drop", "foo" => self.foo).increment(1);
    }
}


#[derive(Debug)]
pub struct ParquetEncoderSkippedField {
    pub field_name: String,
    pub reason: String,
}

impl InternalEvent for ParquetEncoderSkippedField {
    fn emit(self) {
        warn!(
            message = "Mismatched field value skipped",
...
fn get_observo_record_in_schema(parquet_serializer: &ParquetSerializerOptions) -> String {
    if !parquet_serializer.record_complete_event.unwrap_or(false) {
        return parquet_serializer.schema.to_string()
    }
    if let Some((schema_without_closing, _)) = parquet_serializer.schema.rsplit_once( '}') {
        return format!("{} optional binary {}; }}", schema_without_closing, OBSERVO_RECORD);
    } else {
        parquet_serializer.schema.to_string()
    }
}
...
impl ParquetSerializerConfig {
    ...
    pub const fn new(schema: String, record_complete_event: Option<bool>, ignore_type_mismatch_for_optional: Option<bool>) -> Self {
        Self {
            parquet: ParquetSerializerOptions {
                schema,
                record_complete_event,
                ignore_type_mismatch_for_optional,
            },
        }
    }
    ...
    pub fn build(&self) -> Result<ParquetSerializer, BuildError> {
        let schema = parse_message_type(get_observo_record_in_schema(&self.parquet).as_str())
            .map_err(|error| format!("Failed building Parquet serializer: {}", error))?;
        self.validate_logical_schema(&schema)
            .map_err(|error| format!("Failed building Parquet serializer: {}", error))?;
        Ok(ParquetSerializer {
            schema: Arc::new(schema),
            ignore_type_mismatch_for_optional: self.parquet.ignore_type_mismatch_for_optional.unwrap_or(false),
        })
    }
...
pub struct ParquetSerializerOptions {
    ...
    /// Flag to enable observo_record field in parquet schema.
    #[configurable(metadata(docs::examples = r#"Set true to enable capturing complete log event along with the given parquet schema
    Set false to only capturing fileds of log event that match the given parquet schema"#))]
    pub record_complete_event: Option<bool>,

    /// Flag to treat type mismatches in optional fields as null values instead of errors.
    #[configurable(metadata(
        docs::examples = r#"Set true to insert null for optional fields with type mismatches.
    Set false to throw error on any type mismatch (default behavior)"#
    ))]
    pub ignore_type_mismatch_for_optional: Option<bool>,
}
...
pub struct ParquetSerializer {
    schema: TypePtr,
    ignore_type_mismatch_for_optional: bool,
}
...
impl ParquetSerializer {
    fn process<T: DataType>(
        ...
        let mut column = Column::<<T as DataType>::T, _>::new(
            &*self.schema,
            desc,
            extractor,
            self.ignore_type_mismatch_for_optional,
        );
...
struct Column<'a, T, F: Fn(&Value) -> Result<T, ParquetSerializerError>> {
    ...
    // Set true to insert null for optional fields with type mismatches
    ignore_type_mismatch_for_optional: bool,
}

impl<'a, T, F: Fn(&Value) -> Result<T, ParquetSerializerError>> Column<'a, T, F> {
    fn new(schema: &'a Type, column: &'a ColumnDescriptor, extract: F, ignore_type_mismatch_for_optional: bool) -> Self {
    ...
    fn extract_value(&mut self, value: &Value, level: Level) -> Result<(), ParquetSerializerError> {
        if let Some(part) = self.levels.get(level.level) {
            let sub = match value {
                Value::Object(object) => match part.name() {
                    OBSERVO_RECORD => {
                        let value = (self.extract)(&Value::from(serde_json::to_string(object)
                            .expect("Error in serializing json string of log")))?;
                        self.push_value(Some(value), level.descend(part.get_basic_info()));
                        return Ok(())
                    }
                    _ => object.get(part.name()),
                }
                Value::Null => None,
                // Invalid type, error
                value => {
                    if self.ignore_type_mismatch_for_optional && part.is_optional() {
                        ...
                        debug!(message = "Ignore type mismatch.", field_name = %part.name(), internal_log_rate_limit=true);
                        None
                    } else {
            ...
            match (self.extract)(value) {
                Ok(v) => {
                    self.push_value(Some(v), level);
                    Ok(())
                }
                Err(e) => {
                    if self.ignore_type_mismatch_for_optional && self.def_levels.is_some() && level.optional {
                        self.push_value(None, level.undefine());
                        Ok(())
                    } else {
                        Err(e)
...
mod tests {
    ...
    fn validate_data(
        events: Vec<Event>,
        num_columns: usize,
        get_encoder: impl Fn() -> ParquetSerializer,
    ...
    fn validate(
        ...
        validate_data(events, num_columns, || {
            let schema = Arc::new(parse_message_type(schema).unwrap());
            ParquetSerializer {
                schema,
                ignore_type_mismatch_for_optional: false,
            }
        }, validate);
    }
    ...
    fn test_serialize() {
        let message_type = r#"
            ...
            "#;
        // optional binary observo_catch_all;
        ...
        validate(
            get_observo_record_in_schema(&ParquetSerializerOptions {
                schema: message_type.to_string(),
    ...
    #[test]
    fn test_serialize_no_observo_record() {
        let message_type = r#"
            message test {
                required group a {
                    required boolean b;
        ...
        validate(
            get_observo_record_in_schema(&ParquetSerializerOptions {
                schema: message_type.to_string(),
                record_complete_event: Some(false),
                ignore_type_mismatch_for_optional: None
            }).as_str(),
            events,
            7,
    ...
    #[test]
    fn test_serialize_with_observo_record() {
        let message_type = r#"
            message test {
                required group a {
                    required boolean b;
        ...
        validate(
            ...
            |i, path, row_group_reader| match path {
                ...
                OBSERVO_RECORD => {
                    ...
                    assert_column(
                        2,
                        &[Bytes::from("{\"a\":{\"b\":true,\"c\":2},\"d\":{\"e\":3},\"f\":{\"g\":[4,5]},\"h\":\"hello\",\"i\":[{\"j\":6,\"k\":[7.0,8.0]}]}").into(),
                            Bytes::from("{\"a\":{\"b\":false},\"f\":{},\"h\":\"world\",\"i\":[{\"j\":9,\"k\":[10.0]},{\"j\":11}]}").into()],
                        None,
                        Some(&[1,1]),
                        reader,
                    );
    ...
    fn test_bad_map() {
        ...
        let mut encoder = ParquetSerializer {
            schema,
            ignore_type_mismatch_for_optional: false,
        };
    ...
    #[test]
    fn test_bad_map_with_null_true_required() {
        let message_type = r#"
            message test {
                optional group answers (MAP){
                    repeated group key_value {
        ...
        match encoder.encode(events, &mut buffer) {
            Ok(_) => { assert!(false, "Should have failed as value is not boolean."); },
            Err(err) => {
                assert_eq!(err.to_string(), "Event does not contain required field. field = \"answers.key_value.value\"");
            }
        };
    }

    #[test]
    fn test_bad_map_ignore() {
        let message_type = r#"
            message test {
                optional boolean mybool;
                optional int64 myint;
                      ...
                      |i, path, row_group_reader| match path {
                          ...
                          "mystring" => {
                              ...
                              assert_column(
                                  3,
                                  &[
                                      Bytes::from("my_string_value").into(),
                                  ],
                                  Some(&[0, 0, 0]),
                                  Some(&[1, 0, 0]),
                                  reader,
