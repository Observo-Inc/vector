...
impl Aggregator {
    pub fn new(default_api_key: Arc<str>) -> Self {
        Self {
            ...
            oldest_timestamp: align_timestamp(Utc::now().timestamp_nanos_opt().expect("current time should be valid") as u64),
            default_api_key,
    ...
    fn handle_span(
        ...
        let start = match span.get("start") {
            Some(Value::Timestamp(val)) => val.timestamp_nanos_opt().expect("timestamp should be valid") as u64,
            _ => Utc::now().timestamp_nanos_opt().expect("current time should be valid") as u64,
        };
    ...
    pub(crate) fn flush(&mut self, force: bool) -> Vec<ClientStatsPayload> {
        ...
        let now = Utc::now().timestamp_nanos_opt().expect("current time should be valid") as u64;
