...
async fn http_headers() {
    assert_source_compliance(&SOURCE_TAGS, async {
        ...
        let grpc_config = Some(GrpcConfig {
            address: grpc_addr,
            tls: Default::default(),
        });

        let http_config = Some(HttpConfig {
            address: http_addr,
            tls: Default::default(),
...
#[tokio::test]
async fn only_grpc_config() {
    assert_source_compliance(&SOURCE_TAGS, async {
        let env = build_only_grpc_otlp_test_env(LOGS, Some(true)).await;
        let schema_definitions = env
            .config
            .outputs(LogNamespace::Vector)
            .remove(0)
...
pub async fn build_only_grpc_otlp_test_env(
    event_name: &'static str,
    log_namespace: Option<bool>,
) -> OTelTestEnv {
    let grpc_addr = next_addr();
